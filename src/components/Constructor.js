import React from 'react';

function Constructor() {
  return (
    <section className="constructor">
      <a className="constructor__link" href="#">
        <img className="constructor__image" src="https://lh3.googleusercontent.com/proxy/hr3RgLjcRSvdVLLTuiVsyMLVf1nx93IvF-aKx5Hgb8tQ6Gwd9RkFA5l61OMh-T7Tok8gryl0aw6CFpnIK3bgrX7KC_fu21Zv8S1ZxPSU5S31lsPY0fp15etwbGxOPZjvXU_U" alt="Image" />
        <h3 className="constructor__text">
          Lorem ipsum dolor sit amet, consectetur adipiscing elit. Nam mattis massa augue,
          nec tincidunt tortor molestie nec. Fusce vel neque venenatis, ullamcorper magna ut,
          efficitur quam.
        </h3>
      </a>
    </section>
  )
}
export default Constructor;
